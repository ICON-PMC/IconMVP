...
- **Content:** approve → "¡Tu marca fue aprobada en Icon!" with a link to `/marca/panel`; reject → "Tu solicitud en Icon necesita ajustes" including the staff note (if any) and a link to `/onboarding/marca?paso=1`. Links use `NEXT_PUBLIC_APP_URL` or, if unset, the request origin.
...
- **Deploy checklist (cloud):** set `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `EMAIL_FROM` (a sender the SMTP provider allows) and ideally `NEXT_PUBLIC_APP_URL` in Vercel; without them approvals still work but show the "no pudimos enviar el correo" notice.
